...
- Docker 한 줄로 배포
- Dedicated Instance 모델과 궁합 좋음 (고객사 1개 = WebUI 1개)
...
### nomic-embed-text
**한 줄 설명**: 텍스트를 숫자 벡터로 바꾸는 임베딩 모델.

```
"안녕하세요" → [0.12, -0.45, 0.78, ...] (768차원)
```
...
**라이선스**: **Apache 2.0** ✅ 상업 사용 자유 (모델 가중치·코드). 출처: https://huggingface.co/nomic-ai/nomic-embed-text-v1.5
...
**운영 시 고려사항**: [01-architecture.md 섹션 6](01-architecture.md#6-ec2--ollama-운영-고려사항) 참고
...
**왜 필요한가**:
- 우리는 고객사 MySQL을 미러링하지 않고 binlog로 변경사항 추적
...
**우리 시스템에서**:
- 고객사 MySQL이 다른 AWS 계정: **VPC Peering**
- 고객사 MySQL이 온프레미스: **Site-to-Site VPN**
...
> Phase 1+에서 RabbitMQ 도입 시 초기엔 k3s Pod, 트래픽 증가 시 Amazon MQ 전환.
...
**왜 필요한가**:
- Spring Boot Pod이 여러 개일 때 스케줄러 중복 실행 방지
...
> 현재 시스템은 k3s를 사용. ECS Fargate는 대안으로 검토되었으나 채택되지 않음.
...
**한 줄 설명**: EC2 부팅용 디스크 이미지.

```
한 번 세팅한 EC2 (Ollama + 모델 + 설정 다 포함)를
AMI로 저장 → 새 EC2는 30초 만에 동일 환경으로 부팅
```

**우리 시스템에서**:
- Ollama + qwen2.5:14b + nomic-embed-text 사전 설치
- 새 EC2 인스턴스 시작 시 즉시 사용 가능
- Blue/Green 배포에 활용
...
> 현재 시스템은 Jenkins + Helm으로 배포. CodeDeploy는 ECS 환경에서 사용.
...
### Terraform
**한 줄 설명**: HashiCorp의 Infrastructure as Code 도구. 클라우드 인프라를 코드로 관리.
...
**왜 Terraform**:
- 업계 표준 (멀티 클라우드, AWS/GCP/Azure 모두 지원)
- 풍부한 모듈 생태계 (Terraform Registry)
- 변경 사항 미리보기 (`terraform plan`)
- 상태 파일로 현재 인프라 추적 (S3 백엔드)
...
pipeline {
    ...
    stages {
        ...
        stage('Deploy'){ steps { sh 'helm upgrade app ./chart' } }
    }
    post {
...
- Jenkins → AWS STS AssumeRole → 고객사 AWS 계정 → 배포
...
**왜 필요한가**:
- Dedicated Instance 모델에서 고객사마다 AWS 계정이 다름
- 우리 Jenkins (회사 IAM 사용자) → 고객사 계정에 배포해야 함
- 직접 계정 비밀번호 공유 불가 → Cross-Account Role 사용
...
[고객사 AWS 계정]
- IAM Role 생성: RagOperatorRole
- Trust Policy: 우리 회사 AWS 계정 신뢰
- Permissions Policy: VPC, EC2, RDS 등 필요 권한

[배포 시]
1. Jenkins → AWS STS AssumeRole API 호출
   Role ARN: arn:aws:iam::고객사ID:role/RagOperatorRole
2. STS가 임시 자격증명 발급 (1시간 유효)
3. Jenkins가 임시 자격증명으로 고객사 계정에 Terraform 적용
...
### k3s
**한 줄 설명**: Rancher Labs가 만든 경량 Kubernetes 배포판. K8s API 100% 호환.

**기존 K8s vs k3s**:
```
기존 K8s: 메모리 4GB+ 필요, 설치 복잡
k3s:      메모리 512MB, 1MB 바이너리, 5분 설치

→ 작은 규모(300명 사용자)엔 k3s가 적합
→ 학습한 내용은 EKS/K8s 전환 시 그대로 사용
```
...
### Helm
**한 줄 설명**: Kubernetes 패키지 매니저. 앱 배포를 차트로 표준화.
...
**핵심 명령**:
```bash
helm install rag-backend ./chart       # 설치
helm upgrade rag-backend ./chart       # 업데이트
helm rollback rag-backend              # 롤백
helm list                              # 설치된 차트 목록
```

**왜 Helm**:
- K8s YAML 5~10개를 차트 1개로 묶음
- 환경별 변수 오버라이드 쉬움
- 롤백 한 줄
- 공개 차트 풍부 (Prometheus, Grafana 등)

---

### kubectl
**한 줄 설명**: Kubernetes/k3s 클러스터 조작 CLI.

**자주 쓰는 명령**:
```bash
kubectl get pods                    # Pod 목록
kubectl logs spring-boot-abc123     # Pod 로그
kubectl exec -it spring-boot-abc123 -- bash  # Pod 접속
kubectl apply -f deployment.yaml    # YAML 적용
kubectl rollout status deployment/spring-boot  # 배포 상태
```
...
**역할**: k3s 안에 설치되어 모든 Pod의 메트릭을 주기적으로 수집.
...
| 원본 DB | MySQL (Docker, 샘플) | 회사 MySQL | 고객사 MySQL 직접 연결 (binlog, 우리 측 미러 없음) |
...
| 캐시 / 분산 락 | Redis (Docker) | Redis (Docker) | Redis (k3s Pod) — ShedLock 락 저장소 겸용 |
...
| IaC | (없음) | (없음) | Terraform (Cross-Account 배포) |
...
| 스케줄러 | Spring @Scheduled | Spring @Scheduled | EventBridge |
| 배포 | IDE 실행 | Docker run | CodeDeploy |
