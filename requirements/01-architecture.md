...
| 역할 | 로컬 | 개발 서버 | 운영 서버 |
|------|------|----------|---------------------|
| **Web UI** | Open WebUI (Docker) | Open WebUI (Docker) | Open WebUI (Docker Compose) |
| **앱 서버** | Spring Boot (IDE) | Spring Boot (Docker) | Spring Boot (Docker Compose) |
...
| **원본 DB** | MySQL (Docker, 샘플) | 회사 MySQL | 회사 MySQL 직접 연결 (binlog 기반) |
| **벡터 DB** | pgvector (Docker) | pgvector (Docker) | PostgreSQL + pgvector (운영 환경 미정) |
| **네트워크 연결 (회사 MySQL)** | localhost | localhost | 사내 네트워크 직접 연결 |
| **캐시 / 분산 락** | Redis (Docker) | Redis (Docker) | Redis (Docker Compose) |
| **컨테이너 오케스트레이션** | Docker Compose | Docker Compose | Docker Compose (EC2 단일 호스트) |
| **로드밸런서** | (없음) | Nginx | ALB |
| **DNS** | localhost | dev.company.com | `사내 도메인 (미정)) |
| **모니터링** | 콘솔 로그 | Docker logs | Prometheus + Grafana |
...
[사내 서비스 모델과의 궁합]
- 사내 서비스 = Open WebUI 1개
- 멀티 테넌시 불필요 (사내 단일 인스턴스)
- 
```
...
[Prometheus + Grafana] — 앱 메트릭 (EC2 컨테이너 안)
...
| 항목 | 스펙 | 월 비용 |
|------|------|--------|
| EC2 t3.medium × 1 | Docker Compose 호스트 | $50 |
| EC2 g5.xlarge × 1 (Spot) | Ollama 전용 | $220 |
| RDS PostgreSQL Multi-AZ + pgvector | db.t3.small | $50 |
| Redis | Docker 컨테이너 (캐시 + ShedLock) | $0 |
| ALB | 1개 | $20 |
| 네트워크 | 회사 MySQL 연결 | 환경별 상이 |
...
>
> RDS MySQL은 회사 MySQL 직접 연결로 변경 → 미러 불필요 → 비용 $50 절감
...
[즉시 적용]
- GPU Spot 인스턴스 (현재 적용)
- ElastiCache 대신 Docker Compose Redis 컨테이너
- 메시지 큐 미사용 (Phase 0)
...
### 9-1. 인프라 코드화 (IaC) — 인프라 관리 도구

**원칙**: 모든 AWS 리소스는 인프라 관리 도구으로 정의. 콘솔 클릭 금지.

**사내 서비스 모델에서 인프라 관리 도구이 필수인 이유**:
- 고객사마다 동일한 인프라 반복 배포
- 신규 고객 온보딩 30분 만에 완료
- 모든 고객사에 동일한 구성 보장
- 코드로 인프라 차이 추적
...
│
└── helm/
    ├── rag-backend/             ← Spring Boot docker-compose.prod.yml
    ├── open-webui/              ← Open WebUI docker-compose.prod.yml
    ├── ollama/                  ← Ollama docker-compose.prod.yml
    └── monitoring/              ← Prometheus/Grafana 차트
```

**고객사별 인프라 관리 도구 변수 예시**:
```hcl
...
module "rag_instance" {
  ...
  aws_role_arn       = "arn:aws:iam::111122223333:role/DeployRole"
...
[2] AWS 배포 권한
    → 고객사 A의 단일 계정 Role
    → 임시 자격증명 획득 (1시간 유효)
[3] docker compose up (고객사 A 계정에 배포)
[4] docker compose up -d (EC2에 새 이미지 배포)
[5] Discord 알람
```
        ...
        stage('Assume Customer Role') {
            steps {
                withAWS(role: customerRoleArn(params.CUSTOMER)) {
                    stage('인프라 관리 도구 Apply') { /* 고객사 계정 */ }
                    stage('Docker Compose Deploy') { /* 고객사 EC2 */ }
...
**rag-infra/docker-compose.prod.yml 서비스 구성**:
```
services:
  rag-backend    ← Spring Boot API (:8080)
  open-webui     ← 채팅 UI (:3000)
  redis          ← 캐시·Rate Limit·ShedLock
  prometheus     ← 메트릭 수집
  grafana        ← 대시보드
```
...
**배포 명령**:
```bash
# 이미지 사전 Pull (다운타임 최소화)
IMAGE_TAG=v1.2.3 docker compose -f rag-infra/docker-compose.prod.yml pull

# 서비스 재시작
IMAGE_TAG=v1.2.3 docker compose -f rag-infra/docker-compose.prod.yml up -d --remove-orphans

# 롤백: 이전 이미지 태그로 재배포
IMAGE_TAG=v1.2.2 docker compose -f rag-infra/docker-compose.prod.yml up -d
```
...
[4] 우리: 고객사 정보 등록
    ...
    - 변수 설정 (customer_name, aws_role_arn, instance_size, llm_model 등)
        ↓
[5] Jenkins에서 배포 트리거
    Jenkins → 배포 권한 → 고객사 계정 → docker compose up
        ↓
[6] 인프라 자동 구성 (30분 ~ 1시간)
    - VPC, 서브넷, 보안 그룹
    - EC2, RDS Multi-AZ
    - EC2 Docker Compose 호스트
    - ALB + ACM 인증서
...
[7-1] SES 도메인 검증 (메일 발송용)
    - 회사 서버에서 SES 활성화 (ap-northeast-2)
...
☑ EC2 Docker Compose 환경 구축 (단일 고객사)
☑ 임베딩 모델 버전 관리 (스키마)
☑ PII 마스킹 (정규식)
☑ 기본 모니터링 (Prometheus + Grafana)
☑ 인프라 관리 도구 모듈 (재사용 가능)
☑ Jenkins 파이프라인 (단일 계정 배포)
☑ 신규 고객 온보딩 절차 문서화
...
| 사내 서비스 | - | 사내 단일 인스턴스 |
| 단일 계정 Role | - | AWS 계정 간 권한 위임 메커니즘 |
