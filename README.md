...
# 3. 프로덕션 배포 (Jenkins 파이프라인이 SSM RunCommand로 자동 실행)
#    수동 배포 시 — EC2에 SSH(SSM) 접속 후:
IMAGE_TAG=abc1234 docker compose -f /opt/ragvault/docker-compose.prod.yml pull
IMAGE_TAG=abc1234 docker compose -f /opt/ragvault/docker-compose.prod.yml up -d --remove-orphans
```
...
사용자가 SQL을 몰라도, 자연어 질문만으로 회사 MySQL DB를 조회할 수 있다.  
...
고객사마다 **독립된 AWS 계정**에 전용 인프라가 프로비저닝된다 (사내 단일 인스턴스).  
    ...
    subgraph CUSTOMER["🖥️ 운영 서버"]
        ...
        subgraph NETWORK["VPC  10.0.0.0/16"]
            ...
            subgraph APP["Private App Subnet (AZ-a)"]
                subgraph APPNODE["EC2 App Node (t3.medium)\nDocker Compose"]
                    WEBUI["open-webui\n컨테이너"]
                    BACKEND["rag-backend\n컨테이너"]
                    MON["Prometheus\nGrafana"]
                end
...
graph LR
    subgraph VPC["VPC  10.0.0.0/16"]
        ...
        subgraph PRIV_APP["Private App Subnet\nAZ-a"]
            APP2["App Node (Docker Compose)\nt3.medium × 1"]
        end
        subgraph PRIV_GPU["Private GPU Subnet\nAZ-a"]
            GPU2["GPU Node\ng5.xlarge Spot (Ollama)"]
        end
...
flowchart LR
    ...
    J2["Build JAR\nGradle bootJar"]
    J3["Docker Build\n& ECR Push"]
    J5["Docker Compose Deploy\nSSM RunCommand → docker compose up -d"]
    DC["Discord\n성공/실패 알림"]

    DEV --> J1 --> J2 --> J3 --> J4 --> J5 --> DC

    note1["COMPOSE_ONLY=true\n설정 시 J2·J3 스킵"]
    J3 -.->|선택| note1
```
...
| 분류           | 리소스                | 스펙 / 설정                                            |
| -------------- | --------------------- | ------------------------------------------------------ |
| **컴퓨트**     | EC2 App Node          | t3.medium × 1, Private Subnet                          |
...
│   │   ├── ec2.tf             App Node (Docker Compose, t3.medium×1) + GPU Spot Node
...
- `COMPOSE_ONLY=true` 파라미터로 이미지 빌드 없이 docker compose만 재배포 가능
- EC2 인스턴스 ID는 SSM Parameter Store `/rag/{CUSTOMER_ID}/app-instance-id`에서 취득
...
```
document_chunks           — pgvector 임베딩 청크 (1024차원 HNSW 인덱스)
...
│   │   └── customers/template/          신규 서비스 인스턴스 배포 템플릿
│   ├── docker-compose.prod.yml          프로덕션 배포 (rag-backend, open-webui, redis, prometheus, grafana)
│   ├── helm/                            (레거시 — 현재 미사용)
│
