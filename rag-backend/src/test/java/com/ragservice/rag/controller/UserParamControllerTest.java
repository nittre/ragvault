...
class UserParamControllerTest {
    ...
    void setUp() throws Exception {
        ...
        doAnswer(invocation -> {
            ...
            chain.doFilter(req, res);
            return null;
        }).when(adminSessionFilter).doFilter(any(), any(), any());
    ...
    // -------------------------------------------------------------------------
    // 5. PUT — X-User-Email 없으면 400
    // -------------------------------------------------------------------------
    @Test
    @DisplayName("PUT: X-User-Email 헤더 없으면 400")
    @WithMockUser(username = "user@example.com", authorities = "api:chat")
    void updateProfile_no_email_header_returns_400() throws Exception {
        Map<String, Object> body = Map.of("top_k", 10);
