...
export default function SearchConfigPage() {
  ...
  const { data: configs = [], isLoading, error } = useQuery({
    queryKey: ['admin-search-config', dsId],
    queryFn: () => getSearchConfig(dsId),
    enabled: !!dsId,
  })

  const updateMut = useMutation({
    mutationFn: ({ key, value }: { key: string; value: string }) =>
      updateSearchConfig(dsId, key, { value }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-search-config', dsId] })
      setEditing(null)
    },
  })
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">RAG 검색 동작을 제어하는 설정값입니다.</p>
      </div>
