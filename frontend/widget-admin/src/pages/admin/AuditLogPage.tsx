...
import type { AuditLogDto } from '../../api/admin/audit'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 flex flex-col gap-3 overflow-y-auto flex-1">
          <Row label="행위자" value={row.actorEmail} />
          <Row label="액션">
            <ActionBadge action={row.action} />
          </Row>
          <Row label="대상 유형" value={row.targetType || '-'} />
          <Row label="대상 ID" value={row.targetId || '-'} mono />
...
export default function AuditLogPage() {
  ...
  const [selectedRow, setSelectedRow] = useState<AuditLogDto | null>(null)

  const { data, isLoading, error } = useQuery({
    queryKey: ['admin-audit', page],
    queryFn: () => getAuditLogs({ page, size: 30 }),
  })
              ...
              <tbody className="divide-y divide-gray-100">
                {rows.map(row => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-700 text-xs">{row.actorEmail}</td>
                    <td className="px-4 py-3">
                      <ActionBadge action={row.action} />
                    </td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{row.targetType || '-'}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs font-mono">{row.targetId || '-'}</td>
