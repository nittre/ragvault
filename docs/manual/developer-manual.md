...
- 마스킹 이전의 원본 응답은 별도 서비스(`ResponseRawStorageService`)가 단기 저장하며(TTL 30분, ADR-0010), 이 저장은 **마스킹 호출 전에** 이루어져야 한다는 것이 설계 불변식입니다(감사/재현 목적). **SQL(`TextToSqlService`)·HYBRID(`HybridQueryService`)는 이 순서를 지키지만, RAG 경로(`RagService`+`QueryRouterService`)는 현재 이 순서가 뒤집혀 있어 저장되는 "원본"이 이미 마스킹된 텍스트입니다** 
...
ChatController.chatCompletions()
 ...
 └─ QueryRouterService.route(userMessage, history, userEmail, images, fileIds, routingHint, effectiveParams)
     ...
     └─ (route() 정상 반환 시 @Auditable → AuditLogAspect → auditLogService.log(userEmail, "RAG", ...))
...
| 감사/로그 | `@Auditable` + `AuditLogAspect`(`ChatAuditActionResolver` 매핑) | `ConversationLogService.saveAsync()` |
...
13개 파라미터 모두 실제 실행 경로에 배선되어 있습니다(커밋 `cbc671a`, 마이그레이션 `V36`~`V38`로 완료 — 과거엔 대부분 미배선이었으나 현재는 해결됨. 히스토리는 [`docs/known-issues/param-tuning-pipeline-not-wired.md`](../known-issues/param-tuning-pipeline-not-wired.md) 참고):

| 파라미터 | 소비 위치 |
|---|---|
| `top_k` / `similarity_threshold` / `temperature` / `top_p` / `max_tokens` | `RagService.chat()` — `EffectiveParams`에서 직접 추출해 검색·LLM 호출에 사용 |
| `max_context_tokens`(Guard B) | `RagService.chat()` — 컨텍스트 청크 예산 컷 |
| `query_timeout_sec` / `max_result_rows` | `TextToSqlService.query()` → `SqlExecutorService.execute(sql, dsId, timeoutSec, maxRows)` 4-인자 오버로드로 전달(안전망으로 [1,300]초/[1,5000]행 재클램핑) |
| `sql_temperature` / `sql_few_shot_examples`(Guard B) | `TextToSqlService.query()` |
| `force_path` | `QueryRouterService.route()` — 슬래시 커맨드가 없을 때만 폴백으로 적용 |
| `hybrid_synthesis_style` | `HybridQueryService.query()` |
| `max_history_turns` | `ChatController.resolveMaxHistoryMessages()` |

`ParameterResolver`가 Stage 1(관리자 DB 기본값) → Stage 2(요청별 override) → Guard A(범위 클램핑) → Guard B(강제 고정) 순으로 최종 `EffectiveParams`를 계산하며(위 다이어그램과 동일), `V38` 마이그레이션에서 기존에 있던 추가 단계(Stage2=`search_config`, Stage4=`user_param_profiles`, Stage5=`conversation_param_overrides`)를 제거하고 이 2단계+2가드 구조로 단순화했습니다.
...
- **`@Auditable` 어노테이션 + `@Aspect` 기반 자동 기록**입니다 — 마커 어노테이션(`com.ragvault.core.security.Auditable`, core 모듈 공유)을 컨트롤러/서비스 메서드에 붙이면, 각 서비스가 각자 구현한 `AuditLogAspect`(챗: `com.ragservice.rag.aspect`, 위젯: `com.ragvault.widget.aspect`)가 해당 메서드가 예외 없이 정상 반환될 때 자동으로 `AuditLogService.log(...)`를 호출합니다. 어노테이션 속성(`action`/`actor`/`targetType`/`targetId`/`detail` 등)은 SpEL 문자열로 평가되며, 메서드 파라미터(이름으로 접근)·반환값(`#result`)·스프링 빈(`@beanName.method()`)을 참조할 수 있습니다. **신규 관리자 액션을 추가할 때는 메서드에 `@Auditable`만 붙이면 되고, `auditLogService.log(...)`를 직접 호출하는 곳은 코드베이스 어디에도 없습니다.**
- `actor` 속성을 비워두면 Aspect가 `SecurityContextHolder`에서 현재 인증 사용자를 기본 추출합니다(없으면 `"unknown"`). 로그인처럼 액션 시점에 아직 인증 전인 경우에만 요청 파라미터에서 명시적으로 지정합니다(예: `actor = "#req.email"`). IP 주소도 호출부에서 넘기지 않고 Aspect가 `RequestContextHolder`로 직접 추출합니다.
- `@AfterReturning` advice는 컨트롤러뿐 아니라 서비스 메서드에도 붙을 수 있습니다 — 반환값에서 대상 정보를 얻어야 하는 경우(예: 삭제 시 자동 생성 ID 대신 사람이 읽을 수 있는 키를 감사로그에 남기고 싶을 때) 서비스 메서드가 그 값을 반환하도록 하고 거기에 `@Auditable`을 붙입니다. 단, 같은 서비스 메서드가 서로 다른 의미를 갖는 여러 컨트롤러 엔드포인트에서 공유된다면(예: "수정"과 "활성화 토글"이 같은 update 메서드를 호출) 어노테이션은 공유 서비스 메서드가 아니라 **각 컨트롤러 엔드포인트에 개별로** 붙여야 `detail` 등의 문맥 정보가 뒤섞이지 않습니다. 또한 `private` 헬퍼 메서드는 같은 클래스 내부 호출(self-invocation)이라 Spring AOP 프록시를 거치지 않으므로 어노테이션을 붙여도 동작하지 않습니다 — 반드시 프록시 경계를 넘는 외부 호출 지점(컨트롤러의 public 엔드포인트 등)에 붙이세요.
- Aspect의 advice 본문 전체가 try/catch로 감싸져 있어, 감사 로그 저장 과정(SpEL 평가 포함)에서 실패해도 **이미 정상 종료된 비즈니스 로직에는 영향을 주지 않습니다** — `@AfterReturning` advice가 예외를 던지면 그 예외가 호출자에게 그대로 전파되는 Spring AOP의 특성 때문에, 이 try/catch가 빠지면 예컨대 성공한 삭제 요청이 감사로그의 사소한 버그 때문에 500으로 반환되는 회귀가 생길 수 있습니다. Aspect를 수정할 때는 이 보장을 반드시 유지하세요.
- `@Async` + `@Transactional`로 비동기 저장되어 채팅 응답 지연에 영향을 주지 않으며, `AuditLogService.log()` 자체도 저장 실패 시 예외를 삼킵니다.
- 저장되는 요약(`requestSummary`)도 `PiiMasker.mask()`로 마스킹 후 최대 50자만 저장해 원문이 남지 않도록 합니다.
- `ChatAuditActionResolver.resolve()`(챗 서비스, `QueryRouterService.route()`에 붙은 `@Auditable`의 `action` 속성에서 `@chatAuditActionResolver.resolve(#result.intent())` 형태로 SpEL 참조)의 매핑이 어드민 매뉴얼의 "라우팅 상세"(RAG/SQL/파일 업로드/HYBRID/웹 검색/차단/기타) 카테고리와 정확히 대응합니다. "차단"은 별도 action이 아니라 `blocked` boolean 컬럼으로 집계됩니다. **이 매핑 문자열은 `AdminUsageStatsController`가 그대로 쿼리 필터 리터럴로 사용하므로, 값을 바꾸면 사용량 통계가 예외 없이 조용히 0을 반환합니다.**
