-- V32: 비활성(dead) 접근제어 라벨 제거
-- - document_chunks.access_groups : 모든 청크가 ARRAY['all'] 로만 저장되어 필터가 no-op 이었음 (AccessPolicy 제거와 함께 폐기)
-- - datasource_config.is_internal : true 로 세팅되는 경로가 없어 항상 false (FullAccessPolicy 는 무조건 허용이라 무시)
-- 서비스 경계는 프로세스·DB 분리 + 테이블/컬럼 통제(data_sensitivity 등)가 집행한다.
