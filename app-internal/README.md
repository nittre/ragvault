...
- 설정: `rag.sql.query-timeout-sec=10`, `rag.sql.max-rows=1000`
...
- `service/TesseractOcrService(Impl)` — Tess4j OCR
- `service/ImagePathService` — 이미지 경로 처리, S3(24h 임시 저장) 연동
...
| DB | PostgreSQL + pgvector, Flyway(V1~V32 마이그레이션) |
...
> Tika 2.9.2 가 PDFBox 2.x API 를 사용하므로 `build.gradle` 에서 PDFBox 를 `2.0.31` 로 강제 고정합니다.
